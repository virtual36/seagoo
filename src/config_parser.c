...
int
load_config (const char * filename, config_t * cfg)
{
	config_init (cfg);
...
int
store_config (const char * filename, const config_t * cfg)
{
...
int
write_default_config (const char * filename)
{
	config_t cfg;
	config_init (&cfg);
	...
	setting
	    = config_setting_add (root, "graph_pruning_distance", CONFIG_TYPE_INT);
...
int
create_default_config_directory ()
{
